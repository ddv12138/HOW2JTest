...
public interface OrderItemMapper {
    ...
    @Results({
            ...
            @Result(property = "product", javaType = Product.class, column = "pid", one = @One(select = "ProductMapper.getProductById")),
            @Result(property = "order", javaType = Order.class, column = "oid", one = @One(select = "OrderMapper.getOrderById"))
    })
